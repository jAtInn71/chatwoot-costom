class Api::V1::Accounts::InboxesController < Api::V1::Accounts::BaseController
  ...
  def reauthorize_and_update_channel(channel_attributes)
    ...
    # Ensure selected_feature_flags is handled as an array even if passed as a single string
  ...
  def permitted_params(channel_attributes = [])
    ...
    # Wrap selected_feature_flags to permit it as an array
    channel_params = [:type, *channel_attributes]
    if channel_attributes.include?({ selected_feature_flags: [] }) || channel_attributes.include?(:selected_feature_flags)
      # ensure it's handled as an array in permit
      channel_params << { selected_feature_flags: [] }
    end
    params.permit(*inbox_attributes, channel: channel_params)
  end
