...
const replyTimeText = useMapGetter('appConfig/getReplyTimeText');
...
const hasCustomReplyTimeText = computed(
  () => replyTimeText.value && replyTimeText.value.trim().length > 0
);
</script>
...
<template>
  <div class="flex items-center justify-between gap-2">
    <div class="flex flex-col gap-1">
      <div v-if="showHeader" class="font-medium text-n-slate-12 online-status">
        {{ headerText }}
      </div>

      <span
        v-if="hasCustomReplyTimeText"
        :class="textClasses"
        class="text-n-slate-11 reply-time"
      >
        {{ replyTimeText }}
      </span>
      <AvailabilityText
